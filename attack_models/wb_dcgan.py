...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../gan_models/dcgan'))
...
def main():
    ...
    with tf.Session(config=run_config) as sess:
        ...
        if args.initialize_type == 'zero':
            ...
            idx = 0
            init_val['pos'] = np.load(os.path.join(args.nn_dir, 'pos_z.npy'))[:, idx, :]
            init_val['neg'] = np.load(os.path.join(args.nn_dir, 'neg_z.npy'))[:, idx, :]
